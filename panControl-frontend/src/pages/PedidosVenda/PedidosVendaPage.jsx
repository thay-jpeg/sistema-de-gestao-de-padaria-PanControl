...
export default function PedidosVendaPage() {
  ...
  async function entregarPedidoAPI(id) {
    setConfirmAction({
      label: 'Deseja marcar este pedido como entregue?',
      fn: async () => {
        try {
          const response = await fetch(`http://localhost:8080/api/pedidos/${id}/entregar`, {
            method: 'PUT'
          })
          if (response.ok) {
                  ...
                  <tr key={pId} className={`${i % 2 === 0 ? 'bg-white' : 'bg-gray-50'} hover:bg-yellow-50 transition cursor-pointer`}
                    ...
                    <td className="px-4 py-3" onClick={e => e.stopPropagation()}>
                      <div className="flex gap-1">
                        {p.situacao === 'pendente' && (
                          <>
                            <ActionBtn label="Entregar" cls="text-green hover:bg-green/10 border-green/30 mr-1" onClick={() => entregarPedidoAPI(pId)} />
          ...
          <div className="p-6">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {selected.situacao === 'pendente' && (
                <>
                  <Button variant="green" onClick={() => entregarPedidoAPI(selected.idPedidoVenda || selected.id)}>Marcar como Entregue</Button>
