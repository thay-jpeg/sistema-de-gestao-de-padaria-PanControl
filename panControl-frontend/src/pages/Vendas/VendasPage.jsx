...
export default function VendasPage() {
  ...
  const isGestor = canAccess(user?.perfil, 'pedidosVenda');
  ...
  useEffect(() => {
    ...
    // Nova função para buscar os produtos ao abrir a tela
    ...
    fetchClientes();
    fetchProdutos(); // Chama a função aqui!
  }, []);
  ...
  const getProdutoId = (p) => p?.idProduto_PK || p?.idProdutoPK || p?.idProduto || p?.id;

  function addToCart(product, quantity = 1) {
    const q = parseInt(quantity) || 1;
    const price = getPrice(product);
    const prodId = getProdutoId(product); // Pega o ID universal

    setCart(prev => {
      const ex = prev.find(i => getProdutoId(i) === prodId);
      if (ex) {
        return prev.map(i => getProdutoId(i) === prodId
          ? { ...i, qty: i.qty + q, total: (i.qty + q) * price }
          : i
        );
      }
      return [...prev, { ...product, idUnico: prodId, qty: q, price, total: q * price }];
    });
    setProdCode(''); setQty('');
  }

  function removeFromCart(targetId) {
    setCart(prev => prev.filter(i => getProdutoId(i) !== targetId));
  }
  ...
  async function confirmPayment() {
    ...
    const payload = {
      ...
      idUsuario: user?.idUsuario_PK || user?.idUsuario || user?.id || user?.codigoAcesso || 1,
      idClienteAtacadista: cli ? cli.idClienteAtacadista_PK : null, 
      nfeCpf: nfData.cpf || null,
      nfeEmail: nfData.email || null,
      desconto: discount,
      itens: cart.map(item => ({
        idProduto: getProdutoId(item),
  ...
  async function savePedido() {
    ...
    const payload = {
      idUsuario: user?.idUsuario_PK || user?.idUsuario || user?.id || 1,
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-[400px] flex flex-col border-r border-gray-200 bg-gray-50">
          <div className="flex-1 overflow-y-auto pan-scroll divide-y divide-gray-100">
            {cart.map(item => {
              const currentId = getProdutoId(item);
              return (
                <div key={currentId} className="grid grid-cols-4 px-4 py-2 text-sm bg-yellow-50 items-center group">
                  <span className="col-span-2 font-bold truncate text-xs">{item.nomeProduto || item.name}</span>
                  <span className="text-center text-gray-600 text-xs">{item.codigoBarras || item.code}</span>
                  <span className="text-right font-semibold text-xs">{item.qty}</span>
                  <button onClick={() => removeFromCart(currentId)} className="text-right text-red text-xs opacity-0 group-hover:opacity-100">✕</button>
                </div>
              );
            })}
          </div>
      ...
      <Modal isOpen={pedidoOpen} onClose={() => setPedidoOpen(false)} className="w-[580px]">
        <div className="p-6">
          ...
          <select value={pedidoClient} onChange={e => setPedidoClient(e.target.value)} className="w-full border border-gray-200 rounded px-3 py-2 text-sm mb-4">
            ...
            {dbClients.map(c => {
              const clientId = c.idClienteAtacadista_PK || c.idClienteAtacadista || c.id;
              return (
                <option key={clientId} value={clientId}>
                  {c.nomeRazaoSocial || c.nome || 'Cliente sem nome'}
                </option>
              )
            })}
          </select>
