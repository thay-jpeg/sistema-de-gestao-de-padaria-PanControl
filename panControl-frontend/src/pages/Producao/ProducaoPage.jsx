...
export default function ProducaoPage() {
  const { ingredients, products, compras, producoes, addProducao, addPerda, calcValidade, calcCusto, getValidadeIng } = useData()
  ...
  const [dataProducao,setDataProd]   = useState(brToISO(todayBR()))
  ...
  const productForProd = useMemo(
    () => products.find(p => p.id === selectedProductId || p.code === addCode),
    [selectedProductId, addCode, products]
  )

  const validadeLoteCalc = useMemo(() => {
    if (!productForProd) return ''
    return calcValidade(productForProd.fichasTecnica||[], productForProd.diasValidadePadrao, isoToBR(dataProducao))
  }, [productForProd, dataProducao, calcValidade])

  const custoProducaoCalc = useMemo(() => {
    if (!productForProd || !prodQtd) return 0
    return calcCusto(productForProd.fichasTecnica||[]) * (parseFloat(prodQtd)||0)
  }, [productForProd, prodQtd, calcCusto])
  ...
  function saveProducao() {
    ...
    addProducao({
      idProduto: productForProd.id,
      nomeProduto: productForProd.name,
      quantidadeProduzida: parseFloat(prodQtd),
      custoTotalProducao:  custoProducaoCalc,
      dataProducao:        isoToBR(dataProducao),
      dataValidadeLote:    validadeLoteCalc,
      idUsuario:           'USR003',
    })
  ...
  function openRegistrarPerda(prod) {
    setSelProducao(prod)
    setPerdaForm({ ...EMPTY_PERDA, dataPerda: brToISO(todayBR()) })
    setModalPerda(true)
  }
            ...
            <div className="col-span-2 flex flex-col gap-1">
              ...
              <div className="flex gap-2">
                <input value={addCode} onChange={e=>{setAddCode(e.target.value); setSelProdId('')}}
                  onKeyDown={e=>{
                    if(e.key==='Enter'){
                      const found = products.find(p=>p.code===addCode.trim()||p.name.toLowerCase()===addCode.toLowerCase().trim())
                      if(found) setSelProdId(found.id)
                    }
                  }}
                ...
                <Button variant="brown" onClick={()=>{
                  const found = products.find(p=>p.code===addCode.trim()||p.name.toLowerCase().includes(addCode.toLowerCase().trim()))
                  if(found) setSelProdId(found.id)
                  else alert('Produto não encontrado.')
                }}>Buscar</Button>
              </div>
              {/* Lista dropdown rápida */}
              {addCode.length > 1 && !productForProd && (
                <div className="border border-gray-200 rounded-lg overflow-hidden mt-1 max-h-32 overflow-y-auto pan-scroll">
                  {products.filter(p=>p.name.toLowerCase().includes(addCode.toLowerCase())||p.code.includes(addCode)).map(p=>(
                    <button key={p.id} onClick={()=>{setAddCode(p.name);setSelProdId(p.id)}}
